        ...
        public bool CheckLink(DXItemGrid grid)
        {
            ...
            switch (grid.GridType)
            {
                ...
                case GridType.AccessoryRefineLevelTarget:
                    ...
                    if (Item.Level >= (Globals.AccessoryExperienceList.Count - (2 - (int)Item.Info.Rarity) * 2)) return false;
                ...
                case ItemType.System:
                    ...
                    switch (Item.Info.Effect)
                    {
                        case ItemEffect.GenderChange:
                            ...
                            {
                                GameScene.Game.ReceiveChat("穿着盔甲时不能改变性别.", MessageType.System);
                                return false;
                            }
                        ...
                        case ItemEffect.ArmourDye:
                            if (GameScene.Game.CharacterBox.Grid[(int)EquipmentSlot.Armour].Item == null)
                            {
                                GameScene.Game.ReceiveChat("只有穿着盔甲时才能使用燃料.", MessageType.System);
                                return false;
                            }
            ...
            switch (e.Button)
            {
                case MouseButtons.Left:
                    ...
                    if (CEnvir.Shift)
                    {
                        ...
                        DXItemAmountWindow window = new DXItemAmountWindow("道具拆分", Item);
                ...
                case MouseButtons.Right:
                    ...
                    switch (GridType)
                    {
                        ...
                        case GridType.Inventory:
                            ...
                            if (GameScene.Game.NPCRepairBox.IsVisible)
                            {
                                if (Item.CurrentDurability >= Item.MaxDurability || !Item.Info.CanRepair)
                                    GameScene.Game.ReceiveChat($"无法修理 {Item.Info.ItemName}, 该物品无需修理.", MessageType.System);
                                else if (!MoveItem(GameScene.Game.NPCRepairBox.Grid))
                                    GameScene.Game.ReceiveChat($"无法在这里修理 {Item.Info.ItemName}.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.NPCSellBox.IsVisible)
                            {
                                if (!Item.Info.CanSell)
                                    GameScene.Game.ReceiveChat($"无法出售 {Item.Info.ItemName}, 该物品不可出售.", MessageType.System);
                                else if (!MoveItem(GameScene.Game.NPCSellBox.Grid))
                                    GameScene.Game.ReceiveChat($"无法在这里出售 {Item.Info.ItemName}.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.NPCMasterRefineBox.IsVisible)
                            {
                                ...
                                GameScene.Game.ReceiveChat($"无法将 {Item.Info.ItemName} 用于精炼.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.NPCWeaponCraftBox.IsVisible)
                            {
                                ...
                                GameScene.Game.ReceiveChat($"无法将 {Item.Info.ItemName} 用于打造.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.NPCItemFragmentBox.IsVisible)
                            {
                                ...
                                if (!Item.CanFragment())
                                    GameScene.Game.ReceiveChat($"无法分解 {Item.Info.ItemName}, 该物品不可分解.", MessageType.System);
                            ...
                            if (GameScene.Game.NPCAccessoryLevelBox.IsVisible)
                            {
                                if (GameScene.Game.NPCAccessoryLevelBox.TargetCell.Grid[0].Link == null)
                                {
                                    ...
                                    GameScene.Game.ReceiveChat($"无法将 {Item.Info.ItemName} 用于升级.", MessageType.System);
                            ...
                            if (GameScene.Game.NPCAccessoryUpgradeBox.IsVisible)
                            {
                                ...
                                if (!Item.CanAccessoryUpgrade())
                                    GameScene.Game.ReceiveChat($"无法提升 {Item.Info.ItemName}.", MessageType.System);
                                else
                            ...
                            if (GameScene.Game.StorageBox.IsVisible)
                            {
                                if (!MoveItem(GameScene.Game.StorageBox.Grid))
                                    GameScene.Game.ReceiveChat("仓库已满.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.TradeBox.IsVisible)
                            {
                                if (!MoveItem(GameScene.Game.TradeBox.UserGrid))
                                    GameScene.Game.ReceiveChat("无法寄售该物品.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.GuildBox.StorageTab.IsVisible)
                            {
                                if (!MoveItem(GameScene.Game.GuildBox.StorageGrid))
                                    GameScene.Game.ReceiveChat("无法将该物品存放在帮会仓库中.", MessageType.System);
                                return;
                            }
                            ...
                            if (GameScene.Game.CompanionBox.IsVisible)
                            {
                                if (!MoveItem(GameScene.Game.CompanionBox.InventoryGrid))
                                    GameScene.Game.ReceiveChat("伙伴背包已满.", MessageType.System);
                                return;
                            }
                        ...
                        case GridType.CompanionInventory:
                            ...
                            {
                                if (!MoveItem(GameScene.Game.GuildBox.StorageGrid))
                                    GameScene.Game.ReceiveChat("无法将该物品保存在帮会仓库中.", MessageType.System);
                                return;
                            }

                            if (!MoveItem(GameScene.Game.InventoryBox.Grid))
                                GameScene.Game.ReceiveChat("背包已满.", MessageType.System);
                        ...
                        case GridType.GuildStorage:
                            ...
                            if (GameScene.Game.NPCRepairBox.Visible)
                            {
                                if (Item.CurrentDurability >= Item.MaxDurability || !Item.Info.CanRepair)
                                    ...
                                    GameScene.Game.ReceiveChat($"无法在此修理 {Item.Info.ItemName}.", MessageType.System);
                                return;
                            }
